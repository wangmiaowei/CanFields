...
class DeformSamples(torch.utils.data.Dataset):
    def __init__(self,filename,train_step = 1):
        ...
        sub_files = sorted(sub_files, key=lambda x: int(x.split('.')[1]))
        ...
        train_lens = 0
        idx_lens = int(sub_files[-1].split('.')[1])-int(sub_files[0].split('.')[1])+1
        for i in range(0,total_lens,int(train_step)):
            sub_file = sub_files[i]
            this_lens = int(sub_file.split('.')[1])-int(sub_files[0].split('.')[1])+1
            #self.normalized_times.append(torch.tensor(i/total_lens))
            self.normalized_times.append(torch.tensor(this_lens/idx_lens))
...
if __name__ == "__main__":
    path = None
    dataset = DeformSamples(path,train_step = 3)
