...
def digs_gradient(inputs, outputs, create_graph=True, retain_graph=True):
...
class DiGSLoss(nn.Module):
    ...
    def forward(self, output_pred, mnfld_points, nonmnfld_points, mnfld_n_gt=None, nonmnfld_dist=None, curvatures=None,
        ...
        if manifold_pred is not None:
            ...
            mnfld_grad = digs_gradient(mnfld_points, manifold_pred)
        else:
            mnfld_grad = None

        nonmnfld_grad = digs_gradient(nonmnfld_points, non_manifold_pred)
        ...
        if self.use_curvs:
            ...
            mnfld_dx = digs_gradient(mnfld_points, mnfld_grad[:, :, 0])
            mnfld_dy = digs_gradient(mnfld_points, mnfld_grad[:, :, 1])
            if dims == 3:
                mnfld_dz = digs_gradient(mnfld_points, mnfld_grad[:, :, 2])
        ...
        # div_term
        if self.use_div:
            nonmnfld_dx = digs_gradient(nonmnfld_points, nonmnfld_grad[:, :, 0])
            nonmnfld_dy = digs_gradient(nonmnfld_points, nonmnfld_grad[:, :, 1])
            if dims == 3:
                nonmnfld_dz = digs_gradient(nonmnfld_points, nonmnfld_grad[:, :, 2])
