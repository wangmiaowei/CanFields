...
class OF4D(nn.Module):
    ...
    def forward(self, t, cxyz):
        ...
        Args:
            ...
            invert (bool): whether to go backwards
        '''
        p = torch.cat([torch.broadcast_to((t-0.5), (cxyz.shape[0],1)),cxyz],dim=-1)

        net = self.fc_p(p)
        # Layer loop
        for i in range(self.n_blocks):
            if self.c_dim != 0:
                net_c = self.fc_c[i](self.embeddings.weight[0]).unsqueeze(1)
                net = net + net_c.T

            if self.z_dim != 0:
                net_z = self.fc_z[i](self.embeddings.weight[1]).unsqueeze(1)
                net = net + net_z.T
        ...
        if self.compute_gradient:
            total_dxyz_dxyz=[]
            for o in range(3):
                derivs = self.df(motion_vectors[:,o],p)
...
if __name__ == "__main__":
    ...
    veloc = OF4D()
    veloc.create(start_end_vecs)
    velocity = veloc(torch.tensor(0.5),inputs)
